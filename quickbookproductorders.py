...
if selected == 'PO Creater':
    ...
    if uploaded_file_prodlist and uploaded_file_OG:
        ...
        ogdf = pd.read_excel(uploaded_file_OG, sheet_name = 'thisone', header = None)
        ...
        for i in ogtables:
            ...
            sizecostlist =[]
            [sizecostlist.append(x) for x in list(prodlistskuinf[globals()[f'sizelist_{i}']].reset_index(drop = True).iloc[0]) if str(x) != 'nan' else None]
        ...
        for i in PO_dataframes_names:
            ...
            csv_data_po = convert_df_to_csv(globals()[f'{i}'])
